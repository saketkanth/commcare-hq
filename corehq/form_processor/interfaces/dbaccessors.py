...
class CaseAccessors(object):
    ...
    def get_case_ids_in_domain_by_owners(self, domain, owner_ids):
        assert domain == self.domain  # TODO: Get rid of domain arg?
        return self.db_accessor.get_case_ids_in_domain_by_owner(domain, owner_ids)
    ...
    def get_extension_case_ids(self, domain, case_ids):
        assert domain == self.domain  # TODO: Get rid of domain arg?
        return self.db_accessor.get_extension_case_ids(domain, case_ids)

    def get_indexed_case_ids(self, domain, case_ids):
        assert domain == self.domain  # TODO: Get rid of domain arg?
        return self.db_accessor.get_indexed_case_ids(domain, case_ids)

    def get_last_modified_dates(self, domain, case_ids):
        assert domain == self.domain  # TODO: Get rid of domain arg?
        return self.db_accessor.get_last_modified_dates(domain, case_ids)

    def get_closed_case_ids(self, domain, owner_id):
        assert domain == self.domain # TODO: Get rid of domain arg?
        return self.db_accessor.get_closed_case_ids(domain, owner_id)

    def get_all_reverse_indices_info(self, domain, case_ids):
        assert domain == self.domain # TODO: Get rid of domain arg?
        return self.db_accessor.get_all_reverse_indices_info(domain, case_ids)
