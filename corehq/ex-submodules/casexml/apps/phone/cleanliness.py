...
def get_cleanliness_flag_from_scratch(domain, owner_id):
    ...
    if cases_to_check:
        closed_owned_case_ids = set(casedb.get_closed_case_ids(domain, owner_id))
        ...
        if cases_to_check:
            ...
            reverse_index_infos = casedb.get_all_reverse_indices_info(domain, list(cases_to_check))
...
def _get_direct_dependencies(domain, cases):
    ...
    extension_cases = set(case_accessor.get_extension_case_ids(domain, cases))
    indexed_cases = set(case_accessor.get_indexed_case_ids(domain, cases))
...
def get_case_footprint_info(domain, owner_id):
    ...
    """
    open_case_ids = set(CaseAccessors(domain).get_open_case_ids(domain, owner_id))
