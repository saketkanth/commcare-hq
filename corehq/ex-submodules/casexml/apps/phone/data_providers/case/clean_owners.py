    ...
    def _get_case_ids_for_owners_without_extensions(self, owner_id):
        if self.is_clean(owner_id):
            ...
            if self.restore_state.is_initial:
                ...
                return set(CaseAccessors(domain).get_open_case_ids(domain, owner_id))
            else:
                ...
                return set(CaseAccessors(domain).get_case_ids_modified_with_owner_since(
                    domain, owner_id, self.restore_state.last_sync_log.date
                ))
        else:
    ...
    def _get_case_ids_for_owners_with_extensions(self, owner_id):
        ...
        if not self.is_clean(owner_id) or self.restore_state.is_first_extension_sync:
            ...
            if self.restore_state.is_initial:
                ...
                all_case_ids = set(case_accessor.get_open_case_ids(domain, owner_id))
                new_case_ids = set(all_case_ids)
                while new_case_ids:
                    all_case_ids = all_case_ids | new_case_ids
                    extension_case_ids = set(case_accessor.get_extension_case_ids(domain, new_case_ids))
...
def filter_cases_modified_since(domain, case_ids, reference_date):
    ...
    """
    last_modified_date_dict = CaseAccessors(domain).get_last_modified_dates(domain, case_ids)
...
def _move_no_longer_owned_cases_to_dependent_list_if_necessary(restore_state):
    if not restore_state.is_initial:
        ...
        if removed_owners:
            ...
            case_ids_to_try_purging = CaseAccessors(domain).get_case_ids_in_domain_by_owners(domain, list(removed_owners))
