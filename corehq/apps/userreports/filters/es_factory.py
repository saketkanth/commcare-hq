...
def _build_compound_filter(spec, context):
    ...
    filters = [FilterFactoryES.from_spec(subspec, context) for subspec in spec['filters']]
    return compound_type_map[spec['type']](*filters)


def _build_not_filter(spec, context):
    wrapped = NotFilterSpec.wrap(spec)
    return es_filters.NOT(FilterFactoryES.from_spec(wrapped.filter, context))
...
def _build_property_match_filter(spec, context):
    ...
    wrapped.getter  # for validation
    term = wrapped.property_name or '.'.join(wrapped.property_path)
    value_expression = ExpressionFactory.from_spec(wrapped.property_value)
    if not value_expression.type == 'constant':
        raise BadSpecError('unsupported expression type', spec)
    return es_filters.term(term, value_expression.constant)
...
def _build_boolean_expression_filter(spec, context):
    ...
    value_expression = ExpressionFactory.from_spec(wrapped.property_value, context)
    if value_expression.type != 'constant':
        raise BadSpecError('unsupported expression type: {}'.format(expression.type), spec)

    if expression.type == 'property_name':
        term = expression.property_name
    elif expression.type == 'property_path':
        term = '.'.join(expression.property_path)
    else:
        raise BadSpecError('unsupported expression type: {}'.format(expression.type), spec)

    def _eq():
        if value_expression.constant is None:
            return es_filters.missing(term, None, None)
        else:
            return es_filters.term(term, value_expression.constant)

    if wrapped.operator == 'in':
        return es_filters.term(term, value_expression.constant)
    if wrapped.operator == 'eq':
        return _eq()
    elif wrapped.operator == 'not_eq':
        return es_filters.NOT(_eq())
    elif wrapped.operator in ('lt', 'lte', 'gt', 'gte'):
        return es_filters.range_filter(term, **{wrapped.operator: value_expression.constant})
    else:
        raise BadSpecError('unsupported operator', spec)
